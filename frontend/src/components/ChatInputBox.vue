<template>
  <div class="w-full px-4 pb-3 sm:px-8">
    <div class="mx-auto max-w-3xl rounded-2xl border border-slate-200 bg-white p-3 shadow-[0_8px_30px_rgba(42,57,92,.06)]">
      ...
      <div class="mt-1 flex items-center justify-between gap-2"><div class="flex items-center gap-2"><select v-model="model" class="rounded-lg border-0 bg-slate-50 px-2 py-1.5 text-xs text-slate-600 outline-none"><option value="qwen3:1.7b">Qwen3 1.7B 本地</option><option value="qwen3.8-flash">Qwen3.8 Flash 云端</option><option value="qwen3.8-max">Qwen3.8 Max 云端</option></select><button @click="network = !network" :class="network ? 'bg-indigo-50 text-indigo-600' : 'text-slate-400 hover:bg-slate-50'" class="flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs" title="联网搜索"><Search :size="14" />联网</button></div><button @click="submit" :disabled="!draft.trim() || busy" class="grid h-8 w-8 place-items-center rounded-full bg-[#4d6bfe] text-white transition hover:bg-[#3c57de] disabled:cursor-not-allowed disabled:opacity-40" title="发送"><Send :size="15" /></button></div>
...
const modelLabels = { 'qwen3:1.7b': 'Qwen3 1.7B 本地', 'qwen3.8-flash': 'Qwen3.8 Flash 云端', 'qwen3.8-max': 'Qwen3.8 Max 云端' }
