<template>
  <aside :class="[open ? 'w-72' : 'w-0', open ? 'translate-x-0' : '-translate-x-full']" class="fixed inset-y-0 left-0 z-30 flex flex-col overflow-hidden border-r border-slate-200 bg-[#f9fbff] transition-all duration-200 lg:relative lg:translate-x-0">
    <div class="flex min-w-72 items-center gap-3 px-5 py-5">
      <div class="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#4d6bfe] text-white shadow-sm"><SvgIcon name="sparkles" :size="20" /></div>
    ...
    <div class="mt-6 flex min-h-0 min-w-72 flex-1 flex-col px-4">
      ...
      <div ref="historyScroll" @scroll="handleHistoryScroll" class="scroll-soft min-h-0 flex-1 space-y-0.5 overflow-y-auto">
...
const store = useChatStore(); const route = useRoute(); const router = useRouter(); const open = ref(true); const historyScroll = ref(null); const chatPage = ref(1); const chatsHasMore = ref(true); const loadingChats = ref(false)
async function loadChats(page = 1, append = false) { if (loadingChats.value || (append && !chatsHasMore.value)) return; loadingChats.value = true; try { const res = await api.chatHistory(page, 30); store.chats = append ? [...store.chats, ...(res.data || [])] : (res.data || []); chatPage.value = page; chatsHasMore.value = page < (res.pages || 0) } finally { loadingChats.value = false } }
function handleHistoryScroll(event) { const el = event.target; if (el.scrollTop + el.clientHeight >= el.scrollHeight - 60) loadChats(chatPage.value + 1, true) }
